...
功能描述：CAN1初始化
输入参数：tsjw：重新同步跳跃时间单元.范围:1~3;
			    tbs2：时间段2的时间单元.范围:1~8;
			    tbs1：时间段1的时间单元.范围:1~16;
			    brp ：波特率分频器.范围:1~1024;(实际要加1,也就是1~1024) tq=(brp)*tpclk1
			    mode：0,普通模式;1,回环模式;
返 回 值：0-初始化成功; 其他-初始化失败
注意：入口参数(except mode)都不能为0
计算公式：Baud rate=Fpclk1/((tbs1+tbs2+1)*brp); Fpclk1=42M
                =42M/((3+2+1)*6)
						    =1M
**************************************************************************/
u8 CAN1_Mode_Init(u8 tsjw,u8 tbs2,u8 tbs1,u16 brp,u8 mode)
{
	...
	tsjw-=1; //Subtract 1 before setting //先减去1.再设置
	tbs2-=1;
	tbs1-=1;
	brp-=1;
...
功能描述：CAN发送数据
输入参数：id:标准ID(11位)/扩展ID(11位+18位)	    
			    ide:0,标准帧;1,扩展帧
			    rtr:0,数据帧;1,远程帧
			    len:要发送的数据长度(固定为8个字节,在时间触发模式下,有效数据为6个字节)
			    *dat:数据指针.
返 回 值：0~3,邮箱号.0XFF,无有效邮箱
**************************************************************************/
...
{	   
	...
	len&=0X0F; //Get lower 4 bits //得到低4位
...
//		float Vz;
    ...
    command_lost_count=0; //CAN/串口控制指令丢失计数器清零
...
功能描述：CAN1发送一组数据(固定格式:ID为0X601,标准帧,数据帧)
输入参数：msg:数据指针
    			len:数据长度(最大为8)
返 回 值：0,成功; 其他,失败;
**************************************************************************/
