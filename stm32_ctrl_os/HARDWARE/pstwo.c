...
u8 USB_PS2_DataKey(void){
	if(Get_PS2_KEY(SELECT_KEY)==KEY_ON) return PSB_SELECT;
	if(Get_PS2_KEY(LF_ROCKER_KEY)==KEY_ON) return PSB_L3;
	if(Get_PS2_KEY(RT_ROCKER_KEY)==KEY_ON) return PSB_R3;
	if(Get_PS2_KEY(START_KEY)==KEY_ON) return PSB_START;
	if(Get_PS2_KEY(LF_UP)==KEY_ON) return PSB_PAD_UP;
	if(Get_PS2_KEY(LF_RIGHT)==KEY_ON) return PSB_PAD_RIGHT;
	if(Get_PS2_KEY(LF_DOWN)==KEY_ON) return PSB_PAD_DOWN;
	if(Get_PS2_KEY(LF_LEFT)==KEY_ON) return PSB_PAD_LEFT;
	if(Get_PS2_KEY(L2_KEY)==KEY_ON) return PSB_L2;
	if(Get_PS2_KEY(R2_KEY)==KEY_ON) return PSB_R2;
	if(Get_PS2_KEY(L1_KEY)==KEY_ON) return PSB_L1;
	if(Get_PS2_KEY(R1_KEY)==KEY_ON) return PSB_R1;
	if(Get_PS2_KEY(RT_GREEN)==KEY_ON) return PSB_GREEN;
	if(Get_PS2_KEY(RT_RED)==KEY_ON) return PSB_RED;
	if(Get_PS2_KEY(RT_BLUE)==KEY_ON) return PSB_BLUE;
	if(Get_PS2_KEY(RT_PINK)==KEY_ON) return PSB_PINK;
	else return 0;
...
void USB_PS2_Read(void){
	...
	if( usb_ps2_ready==1 )
	{
		...
		if(Strat&&(PS2_LY<118)&&PS2_ON_Flag==0&&SysVal.Time_count>=CONTROL_DELAY)
			...
			//Start按键按下后，还需要将右摇杆向前推，才可以正式PS2控制小车
	...
	else
	{
		...
		{
			PS2_ON_Flag=0;
			Move_X=0,Move_Y=0,Move_Z=0;//控制超时，将目标速度置0
		}
		PS2_RX = 128;
		PS2_RY = 128;
		PS2_LX = 128;
...
void pstwo_task(void *pvParameters)
{
    ...
    {	
		...
		else if( SysVal.HardWare_Ver==V1_1 )//新款usb接口ps2手柄读取
		{
			USB_PS2_Read();
		} 
		//检测是否需要进入初始位置微调模式
...
功能描述：判断是否为红灯模式,0x41=模拟绿灯，0x73=模拟红灯
输入参数：无
返 回 值：0：红灯模式，其他：其他模式
**************************************************************************/	
...
void PS2_ReadData(void)
{
	...
	PS2_Cmd(Comd[0]);  //Start orders //开始命令
	PS2_Cmd(Comd[1]);  //Request data //请求数据
	for(byte=2;byte<9;byte++) //Start receiving data //开始接收数据
	{
...
功能描述：对读出来的PS2的数据进行处理,只处理按键部分 
输入参数：无
返 回 值：0: 只有一个按键按下时; 1: 未按下
**************************************************************************/	
u8 PS2_DataKey()
{
	u8 index;
...
Output  : none
功能描述：手柄震动函数
输入参数：motor1:右侧小震动电机 0x00关，其他开
	        motor2:左侧大震动电机 0x40~0xFF 电机开，值越大 震越重
返 回 值：无
******************************************************/
...
void PS2_TurnOnAnalogMode(void)
{
	...
	PS2_Cmd(0x03); //0x03 lock storage setup, which cannot be set by the key "mode" set mode. //0x03锁存设置，即不能通过按键“MODE”设置模式。
				         //0xee non-locking software Settings can be set by the key "mode" set mode.//0xEE不锁存软件设置，可以通过按键“MODE”设置模式。
	PS2_Cmd(0X00);
	PS2_Cmd(0X00);
	PS2_Cmd(0X00);
	PS2_Cmd(0X00);
...
void Servo_init_angle_adjust_mode_check(void)
{
	static int check_count1=0,check_count2=0;
		...
		if(PS2_LX<100 && PS2_RX>200)   check_count1++;  //手柄左摇杆向左、右摇杆向右
	  else check_count1=0;
	
		if(PS2_LX>200 && PS2_RX<100)   check_count2++;  //手柄左摇杆向右、右摇杆向左
	  else check_count2=0;
	//进入模式检测
	if(check_count1>500)//保持按住5秒
	{
		
		Beep_Control_Flag = 1;
		Servo_init_angle_adjust=1; //进入舵机初始位置微调模式
		check_count1=0;
		
	}
		//退出模式检测
	if(check_count2>500 && Servo_init_angle_adjust==1)//保持按住5秒
	{
		Beep_Control_Flag = 1;
...
void Servo_init_angle_adjust_function(void)
{
	...
	if (once == 0)
	{
	 ...
	 else if(PS2_KEY==12)		 Moveit_Angle6_init-=step;    //机械夹爪闭合
	 ...
	 else if(PS2_KEY==14)		Moveit_Angle5_init+=step;   //手爪机械臂的旋转角度
