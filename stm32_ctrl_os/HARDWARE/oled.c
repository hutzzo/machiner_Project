...
输入参数：x,y :起点坐标; len :数字的位数; size:字体大小; mode:0,反白显示,1,正常显示	   
返 回 值：无
**************************************************************************/
...
Output  : none
功能描述：显示2个数字
输入参数：x,y :起点坐标; len :数字的位数; size:字体大小; mode:模式, 0,填充模式, 1,叠加模式; num:数值(0~4294967295);	 
返 回 值：无
**************************************************************************/
...
{ 	
  GPIO_InitTypeDef GPIO_InitStructure;
	...
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL;//上拉 初始化GPIO
  ...
  //允许修改RTC和后备寄存器
	...
	//关闭外部低速外部时钟信号功能 也就是PC13 PC14 PC15 都可以当做普通IO用
...
功能描述：显示汉字	
输入参数: x：表示显示的水平坐标; y: 表示显示的垂直坐标;
          no: 表示要显示的汉字（模组）在hzk[][]数组中的行号,通过行号来确定要在数组中显示的汉字,
              注意项目font_width的值，必须与取模软件生成字模时的点阵值大小一致;
          font_height:为字模软件生成字模时的高,因为我的屏幕分辨率为32*128-----0~7共8页，每页4个位
返 回 值：无
注意：此方法显示汉字，一定要满足字模生成软件生成的字库大小相同，如果不相同，请用下一函数
**************************************************************************/	    
...
{ 	
 	unsigned int j=0; //计数变量
 	unsigned char x,y; //坐标变量
  
 	if(y1%8==0) y=y1/8;   //判断截止页是否为8的倍数
 	 else y=y1/8+1;

	for(y=y0;y<y1;y++) //从起始页开始，到截止页
	{
		OLED_Set_Pos(x0,y); //从页的起始行开始
		for(x=x0;x<x1;x++) //从x1 - x0 行
			{
