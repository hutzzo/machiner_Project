...
void TIM8_Cap_Init(u16 arr, u16 psc)
{
	...
	/*** 初始化TIM8输入捕获参数 通道1 || Initialize TIM8 for the capture parameter, channel 1 ***/
	//Select input //选择输入 
  ...
  //IC1F=0000 Configure input filter //配置输入滤波器
	...
	/*** 初始化TIM8输入捕获参数 通道2 || Initialize TIM8 for the capture parameter, channel 2 ***/
	//CC1S=01 Select input //选择输入 
	...
	/*** 初始化TIM8输入捕获参数 通道3 || Initialize TIM8 for the capture parameter, channel 3 ***/
	//Select input //选择输入 
	...
	/*** 初始化TIM8输入捕获参数 通道4 || Initialize TIM8 for the capture parameter, channel 4 ***/
	//Select input //选择输入 
  ...
  //Preempt priority 0 //抢占优先级0	
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 2;  
	//Level 0 from priority //子优先级2
	NVIC_InitStructure.NVIC_IRQChannelSubPriority = 2; 
	//IRQ channels are enabled //IRQ通道使能
  ...
  //允许CC1IE,CC2IE,CC3IE,CC4IE捕获中断	
	TIM_ITConfig(TIM8, TIM_IT_CC1|TIM_IT_CC2|TIM_IT_CC3|TIM_IT_CC4,	ENABLE);   
	//Advanced timer output must be enabled //高级定时器输出必须使能		
...
Input   : none
Output  : none
功能描述：航模遥控接收中断，即定时器8输入捕获中断
输入参数：无
返 回 值：无
**************************************************************************/ 
void TIM8_CC_IRQHandler(void)
{
	//连接航模遥控器后，需要往下拉前进杆，才可以正式控制航模小车
...
//	//Channel 1 //通道1
	if ((TIM8CH1_CAPTURE_STA & 0X80) == 0) 			
	{
		if (TIM_GetITStatus(TIM8, TIM_IT_CC1) != RESET) //A capture event occurred on channel 1 //通道1发生捕获事件
		{
			...
			{
				TIM8CH1_CAPTURE_DOWNVAL = TIM_GetCapture1(TIM8); //Record the timer value at this point //记录此时定时器的计数值
				if (TIM8CH1_CAPTURE_DOWNVAL < TIM8CH1_CAPTURE_UPVAL)
				{
					...
					TIM8_T1 = 0;
				Remoter_Ch1 = TIM8CH1_CAPTURE_DOWNVAL - TIM8CH1_CAPTURE_UPVAL + TIM8_T1;	//Time to get the total high level //得到总的高电平时间
			...
			else 
			{
				...
				//还未开始，第一次捕获上升沿，记录此时定时器的计数值
				TIM8CH1_CAPTURE_UPVAL = TIM_GetCapture1(TIM8); //Obtain rising edge data //获取上升沿数据
				TIM8CH1_CAPTURE_STA |= 0X40; //The flag has been caught on the rising edge //标记捕捉到了上升沿
  ...
  //Channel 2 //通道2
	if ((TIM8CH2_CAPTURE_STA & 0X80) == 0)		
	{
		if (TIM_GetITStatus(TIM8, TIM_IT_CC2) != RESET)	//A capture event occurred on channel 2 //通道2发生捕获事件
		{
			...
			{
				TIM8CH2_CAPTURE_DOWNVAL = TIM_GetCapture2(TIM8); //Record the timer value at this point //记录此时定时器的计数值
				if (TIM8CH2_CAPTURE_DOWNVAL < TIM8CH2_CAPTURE_UPVAL)
				{
					...
					TIM8_T2 = 0;
				Remoter_Ch2 = TIM8CH2_CAPTURE_DOWNVAL - TIM8CH2_CAPTURE_UPVAL + TIM8_T2; //Time to get the total high level //得到总的高电平时间
			...
			else 
			{
				...
				TIM8CH2_CAPTURE_STA |= 0X40; //The flag has been caught on the rising edge //标记捕捉到了上升沿
  ...
  //Channel 3 //通道3
	if ((TIM8CH3_CAPTURE_STA & 0X80) == 0)			
	{
		if (TIM_GetITStatus(TIM8, TIM_IT_CC3) != RESET)	//A capture event occurred on channel 3 //通道3发生捕获事件
		{
			...
			{
				TIM8CH3_CAPTURE_DOWNVAL = TIM_GetCapture3(TIM8); //Record the timer value at this point //记录此时定时器的计数值
				if (TIM8CH3_CAPTURE_DOWNVAL < TIM8CH3_CAPTURE_UPVAL)
				{
					...
					TIM8_T3 = 0;
				Remoter_Ch3 = TIM8CH3_CAPTURE_DOWNVAL - TIM8CH3_CAPTURE_UPVAL + TIM8_T3; //Time to get the total high level //得到总的高电平时间
			...
			else 
			{
				...
				TIM8CH3_CAPTURE_STA |= 0X40; //The flag has been caught on the rising edge //标记捕捉到了上升沿
...
//	
		...
		if ((TIM8CH4_CAPTURE_STA & 0X80) == 0)		
		{
			if (TIM_GetITStatus(TIM8, TIM_IT_CC4) != RESET)	//A capture event occurred on channel 4 //通道4发生捕获事件
			{
				...
				{
					TIM8CH4_CAPTURE_DOWNVAL = TIM_GetCapture4(TIM8); //Record the timer value at this point //记录此时定时器的计数值
					if (TIM8CH4_CAPTURE_DOWNVAL < TIM8CH4_CAPTURE_UPVAL)
					{
						...
						TIM8_T4 = 0;
					Remoter_Ch4 = TIM8CH4_CAPTURE_DOWNVAL - TIM8CH4_CAPTURE_UPVAL + TIM8_T4; //Time to get the total high level //得到总的高电平时间
				...
				else 
				{
				  ...
				  //还未开始，第一次捕获上升沿，记录此时定时器的计数值
					...
					TIM8CH4_CAPTURE_STA |= 0X40; //The flag has been caught on the rising edge //标记捕捉到了上升沿
