...
/**
  ...
  *         接收设备对中断IN令牌的响应
  * @param  pdev: 选定的设备
  * @param  buff: 需要复制响应的缓冲区指针
  * @param  length: 要接收的数据的长度
  * @param  hc_num: 主机通道号
  * @retval 状态
  */
                                ...
                                uint8_t hc_num)
{
  ...
  pdev->host.hc[hc_num].xfer_len = length;
	...
	//无线ps2手柄时序需要提前反转
	if(ps2_type==_2_4G_Android_PS2 || ps2_type==_2_4G_PC_PS2)
	{
  ...
  //有线ps2手柄、或者其他未知设备。按正常usb时序
  if( ps2_type==Normal_PS2 || ps2_type==0)
  {
	  /* toggle DATA PID */
	  pdev->host.hc[hc_num].toggle_in ^= 1;  
  }
