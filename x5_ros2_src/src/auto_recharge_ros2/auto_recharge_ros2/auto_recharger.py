...
pkg_share = get_package_share_directory('auto_recharge_ros2')
default_json = os.path.join(pkg_share, 'Charger_Position.json')
default_yaml = os.path.join(pkg_share, 'robot_info.yaml')
...
class AutoRecharger(Node):
	def __init__(self):
		...
		#读取json文件内保存的充电桩位置信息
		self.declare_parameter('charger_position_json', default_json)
		self.declare_parameter('robot_info_yaml', default_yaml)
		json_file = self.get_parameter('charger_position_json').get_parameter_value().string_value
		yaml_file = self.get_parameter('robot_info_yaml').get_parameter_value().string_value
