...
void turn_on_robot::Moveit_Joint_Callback(const std_msgs::msg::Float32MultiArray::SharedPtr msg)
{
  uint8_t tx[16];
  tx[0]=MOVEIT_HEADER;
  for(int j=0;j<6;j++){
    float rad=(msg->data.size()>j)?msg->data[j]:0.0f;
    short v=(short)round(rad*1000.0f);
    tx[1+2*j]=(uint8_t)((v>>8)&0xFF);
    tx[2+2*j]=(uint8_t)(v&0xFF);
  }
  uint8_t mode=1;
  if(msg->data.size()>=7){
    mode=(uint8_t)round(msg->data[6]);
  }
  tx[13]=mode;
  uint8_t c=0;
  for(int k=0;k<=13;k++) c^=tx[k];
  tx[14]=c;
  tx[15]=MOVEIT_TAIL;
  try{Stm32_Serial.write(tx,16);}catch(serial::IOException& e){}
}
/**************************************
Date: January 28, 2021
...
turn_on_robot::turn_on_robot():rclcpp::Node ("wheeltec_robot")
{
  ...
  Moveit_Sub = create_subscription<std_msgs::msg::Float32MultiArray>(
      "stm32_moveit_joint_cmd", 10, std::bind(&turn_on_robot::Moveit_Joint_Callback, this, std::placeholders::_1));
